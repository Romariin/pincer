...
async function waitForProxyUrl(
  ...
  const ready = (async (): Promise<number> => {
    while (true) {
      ...
      if (done) throw new Error(`Pincer exited before becoming ready. Output: ${output}`);
  ...
  const earlyExit = exited.then((code) => {
    throw new Error(`Pincer exited with code ${code} before becoming ready. Output: ${output}`);
  });
  // External-process integration needs a wall-clock guard so a broken child cannot hang the suite.
  let timer: Timer | undefined;
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new Error(`Timed out waiting for Pincer. Output: ${output}`)), 15_000);
  });
  try {
