...
export class Git {
	...
	private async run(args: string[], signal?: AbortSignal): Promise<GitResult> {
	...
	async isInsideWorkTree(signal?: AbortSignal): Promise<boolean> {
		const r = await this.run(["rev-parse", "--is-inside-work-tree"], signal);
	...
	async rawDiff(paths: string[] = [], signal?: AbortSignal): Promise<string> {
		const r = await this.run(
			["diff", "--no-color", ...(paths.length ? ["--", ...paths] : [])],
			signal,
		);
