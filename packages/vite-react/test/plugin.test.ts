...
test("injects daemon and app identity without legacy shortcut configuration", () => {
  ...
  if (typeof hook !== "function") throw new Error("expected transformIndexHtml hook");
  ...
  if (!configTag.children.startsWith(prefix)) throw new Error("expected Pincer config assignment");
