...
test("shortcut capture cancels on Escape", () => {
	expect(shortcutFromEvent({ ...baseEvent, code: "Escape" })).toEqual({
		type: "cancel",
	});
});

test("shortcut capture ignores repeats and bare modifiers", () => {
	expect(shortcutFromEvent({ ...baseEvent, repeat: true })).toEqual({
		type: "ignore",
	});
	for (const code of [
		"AltLeft",
		...
		expect(shortcutFromEvent({ ...baseEvent, code })).toEqual({
			type: "ignore",
		});
	}
});
