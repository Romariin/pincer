import { MIN_HOST_VISIBLE, PANEL_MAX_W, PANEL_MIN_W, PANEL_W } from "./constants";
...
export function fitPanelWidth(width: number, viewport: number): number {
  return Math.min(clampPanelWidth(width), Math.max(PANEL_MIN_W, viewport - MIN_HOST_VISIBLE));
}

export function loadPanelWidth(): number {
  try {
    const raw = globalThis.localStorage?.getItem(STORAGE_KEY);
    return raw === null || raw === undefined ? PANEL_W : clampPanelWidth(Number(raw));
  } catch {
    return PANEL_W;
  }
}
