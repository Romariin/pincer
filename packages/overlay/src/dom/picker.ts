...
import { formatSourceAttr, parseSourceAttr, SOURCE_ATTR } from "@pincer/core";

const TSD_SOURCE_ATTR = "data-tsd-source";

function resolveAttributeSource(
	node: HTMLElement,
	attribute: string,
): SourceLocation | null {
...
export function elementReference(node: HTMLElement): string {
	const source =
		resolveSource(node) ?? resolveAttributeSource(node, TSD_SOURCE_ATTR);
	if (source)
		return formatSourceAttr({
			...source,
