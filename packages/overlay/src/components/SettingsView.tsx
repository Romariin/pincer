...
export function SettingsView(): ReactNode {
	...
	const startRecordingShortcut = usePincerStore(
		(state) => state.startRecordingShortcut,
	);
	const cancelRecordingShortcut = usePincerStore(
		(state) => state.cancelRecordingShortcut,
	);
	const updateShortcut = usePincerStore((state) => state.updateShortcut);
	const updateShowFloatingButton = usePincerStore(
		(state) => state.updateShowFloatingButton,
	);
	...
	return (
		<div className="pcr-scroll min-h-0 flex-1 overflow-y-auto p-4 pt-[52px]">
			<div className="space-y-6">
				<section
					aria-labelledby="pincer-shortcut-heading"
					className="space-y-3"
				>
				...
				<section
					aria-labelledby="pincer-launcher-heading"
				...
				>
					...
					<label className="flex min-h-11 cursor-pointer items-center gap-3 text-sm">
						<input
							...
							disabled={controlsDisabled}
							onChange={(event) =>
								updateShowFloatingButton(event.currentTarget.checked)
							}
						/>
				...
				<div className="min-h-5 text-xs" aria-live="polite">
					{!connected && (
						<p className="text-muted-foreground">
							Connect to the Pincer daemon to change settings.
						</p>
					)}
