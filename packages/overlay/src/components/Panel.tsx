...
export function Panel(): ReactNode {
	...
	useEffect(() => {
		...
		const body = document.body;
		const apply = (): void => {
			body.style.transition = resizing
				? "none"
				: "margin-right 0.5s cubic-bezier(.32,.72,0,1)";
			...
			const push =
				window.innerWidth < 600
					? 0
					: fitPanelWidth(panelWidth, window.innerWidth);
	...
	useEffect(() => {
		const onKeyDown = (e: KeyboardEvent): void => {
			...
			if (e.key === "Escape") {
				...
				else if ((s.view === "settings" || s.view === "chat") && s.panelOpen)
					s.setView("list");
	...
	const style: CSSProperties = {
		...
		transition: resizing
			? "none"
			: "transform .5s cubic-bezier(.32,.72,0,1),opacity .35s ease",
		fontSize: 14,
	...
	return (
		...
		>
			<OverlayContainerProvider value={pickerContainer}>
				...
				<div
					ref={pickerContainer}
					className="pointer-events-none absolute inset-0 contain-layout"
				/>
