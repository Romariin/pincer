...
import { Message as MessageRoot, MessageAvatar, MessageContent } from "./ui/message";
...
function toolIcon(name: string): ReactNode {
  ...
  if (n.includes("read") || n.includes("view") || n.includes("cat")) return <FileTextIcon />;
  if (n.includes("edit") || n.includes("patch") || n.includes("apply") || n.includes("update")) return <PencilIcon />;
  if (n.includes("write") || n.includes("create")) return <FilePlusIcon />;
  if (n.includes("bash") || n.includes("shell") || n.includes("exec") || n.includes("command") || n.includes("run") || n.includes("terminal"))
    return <TerminalIcon />;
  if (n.includes("grep") || n.includes("search")) return <SearchIcon />;
  if (n.includes("glob") || n.includes("find") || n.includes("list") || n === "ls") return <FolderSearchIcon />;
  if (n.includes("fetch") || n.includes("web") || n.includes("http") || n.includes("url")) return <GlobeIcon />;
    ...
    return (
      <Marker key={i}>
        ...
        <MarkerContent>
          ...
          {block.detail ? <span className="ml-1.5 font-mono text-primary">{block.detail}</span> : null}
        </MarkerContent>
      </Marker>
    );
...
export function Message({ msg, streaming = false }: { msg: Msg; streaming?: boolean }): ReactNode {
      ...
      <MessageAvatar className="self-start bg-transparent">
        {meta ? (
          <Tooltip>
            ...
            <TooltipContent container={overlayContainer ?? undefined} side="bottom" align="start">
              agent {modelLabel(info, meta.model)} · effort {effortLabel(meta.effort)}
            </TooltipContent>
          </Tooltip>
        ) : (
          <Avatar info={info} size={28} />
