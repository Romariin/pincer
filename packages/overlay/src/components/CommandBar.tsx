...
export function CommandBar(): ReactNode {
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 font-semibold text-foreground">{info.label}</span>
      <ChevronRightIcon className="size-3.5 shrink-0 text-muted-foreground/50" />
      <span className="min-w-0 truncate text-muted-foreground">{modelLabel(info, cfg.model)}</span>
      <ChevronRightIcon className="size-3.5 shrink-0 text-muted-foreground/50" />
      <span className="shrink-0 font-semibold text-primary">{effortLabel(cfg.effort)}</span>
